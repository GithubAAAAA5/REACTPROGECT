...
function App() {
  ...
  // useEffect() 사용
  // 화면 렌더링 완료시 데이터 처리를 위해서 사용한다.
  // 이때 비동기 작업을 컴포넌트에서 바로 쓰고, state 를 변경하면 무한 루프에 빠진다.
  ...
  // async, await 규칙
  // 1. async 함수 안에 await 가 존재한다.
  // 2. function 앞에 async 키워드를 추가. 함수는 언제나 Promise 를 반환한다.
  // 3. 리턴이 프로미스라면, await 적용하고, then 을 없앨 수 있다.
  ...
  return (
    <div>
      ...
      <button type='button' onClick={handleClick2}>데이터 로드</button>
      {raw !== undefined?
        <div>
          {raw.userId}<br />
          {raw.userPw}<br />
